...
exports.createXLGen = function (path,options){
	...
	function buildSAT(bookStreamLen){
		...
		for(let i=0; i<sat.length; i++) sat[i]=SID_FREE_SECTOR;
		...
		for(let i = 0; i< 128 * satSectCount ;i++) bufsat.i32(sat[i]);

		bufmsat1 =  new Buf(109*4);
		var len = Math.min(satSect.length, satSect.length)
		for(let i=0; i < len; i++) bufmsat1.i32(satSect[i]);
		bufmsat1.fil(SID_FREE_SECTOR, (109 - i)*4);

		bufmsat2 =  new Buf(msatSectCount*128*4);
		for(let i=0; i< msatSectCount*128; i++) bufmsat2.i32(SID_FREE_SECTOR);
		if(msatSectCount > 0) {
